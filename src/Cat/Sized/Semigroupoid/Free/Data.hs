...
-- data SGF (k ∷ (Nat → κ → κ) → Nat → Nat → κ → κ → Type) (φ ∷ Nat → κ → κ)
--         (n ∷ Nat) (m ∷ Nat)
--         (a ∷ κ)   (b ∷ κ)   where
...
--       -- , ObjectOf φ k n a, ObjectOf φ k m b, ObjectOf φ k l x
--       )
--       ⇒ x -| SGF k φ l m |-> b  -- ^ A free /k/-morphism from /φ l x/ to /φ m b/.
--       → a -| SGF k φ n l |-> x  -- ^ A free /k/-morphism from /φ n a/ to /φ l x/.
--       → a -| SGF k φ n m |-> b
-- instance HFunctor
